...
double moonphase(double ud) {
  ...
  double elonge =
      278.833540; /* Ecliptic longitude of the Sun at epoch 1980.0 */
  ...
  double M = torad *
             fmod(fmod((((360 / 365.2422) * Day) + elonge - elongp), 360) + 360,
                  360); /* Convert from perigee co-ordinates to epoch 1980.0 */
  ...
  double Ec = e;
  Ec = 2 *
       atan(sqrt((1 + eccent) / (1 - eccent)) * tan(Ec / 2)); /* True anomaly */

  double Lambdasun = fmod(fmod((((Ec) * (180.0 / M_PI)) + elongp), 360) + 360,
                          360); /* Sun's geocentric ecliptic longitude */
  double ml = fmod(fmod((13.1763966 * Day + 64.975464), 360) + 360,
                   360); /* Moon's mean lonigitude at the epoch */
  ...
  double lP = ml + Ev + (6.2886 * sin(MmP)) - Ae +
              (0.214 * sin(2 * MmP)); /* Corrected longitude */
  double lPP =
      lP + (0.6583 * sin(torad * (2 * (lP - Lambdasun)))); /* True longitude */
